...
@section('content')
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div id="category-section" class="mb-12 flex flex-wrap gap-3 justify-center items-center px-4 py-8 bg-gradient-to-r from-slate-50 to-orange-50 rounded-2xl border border-slate-200">
            ...
            <a href="/" 
               class="px-6 py-2.5 rounded-full font-semibold text-sm transition-all duration-300 transform hover:scale-105 {{ !request('category') ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-white text-slate-700 border border-slate-300 hover:border-orange-400 hover:bg-orange-50' }}">
                ✓ Semua Kategori
            </a>
            
            @foreach($categories as $cat)
                <a href="/?category={{ $cat->slug }}" 
                    class="px-6 py-2.5 rounded-full font-semibold text-sm transition-all duration-300 transform hover:scale-105 {{ request('category') === $cat->slug ? 'bg-orange-500 text-white shadow-lg shadow-orange-200' : 'bg-white text-slate-700 border border-slate-300 hover:border-orange-400 hover:bg-orange-50' }}">
                    {{ $cat->name }}
                </a>
            @endforeach
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($organizations as $org)
                <a href="{{ route('organizer.profile', $org->slug) }}" class="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:border-orange-200 transition-all duration-300 p-8 block">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-bold text-lg group-hover:text-orange-600 transition truncate">{{ $org->name }}</h3>
                            <p class="text-xs text-slate-400">{{ $org->total_events }} Event</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t">
                        <div class="flex items-center gap-1">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-4 h-4 {{ $i <= round($org->avg_rating) ? 'text-yellow-400' : 'text-slate-200' }}" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                </svg>
                            @endfor
                        </div>
                        <span class="text-sm font-bold {{ $org->total_reviews > 0 ? 'text-orange-600' : 'text-slate-400' }}">
                            {{ $org->total_reviews > 0 ? number_format($org->avg_rating, 1) . ' (' . $org->total_reviews . ' ulasan)' : 'Belum ada ulasan' }}
                        </span>
                    </div>
                </a>
            @empty
