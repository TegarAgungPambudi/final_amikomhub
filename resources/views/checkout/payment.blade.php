...
<main class="max-w-3xl mx-auto px-6 py-20 text-center">
    <div class="bg-white rounded-3xl border border-slate-200 p-12 shadow-sm inline-block w-full max-w-md">
        ...
        <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 mb-8">
            ...
            <h3 class="text-4xl font-extrabold text-orange-600">Rp {{ number_format($transaction->total_price, 0, ',', '.') }}</h3>
            <p class="text-xs text-slate-400 mt-2">Order ID: {{ $transaction->order_id }}</p>
        </div>

        <button id="pay-button" class="w-full py-5 bg-orange-500 text-white rounded-2xl font-black text-xl shadow-xl shadow-orange-200 hover:bg-orange-600 transition animate-bounce-in">
            Bayar Sekarang
        </button>
    </div>
</main>
