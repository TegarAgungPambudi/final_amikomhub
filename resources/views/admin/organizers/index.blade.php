...
@section('page_subtitle', 'Daftar semua penyelenggara event')

@section('content')
<div class="flex justify-between items-center mb-8">
    <div>
        <p class="text-slate-500">Total {{ $organizers->count() }} akun terdaftar</p>
    </div>
    <a href="{{ route('admin.organizers.create') }}" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition flex items-center gap-2">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-bold">{{ $organizer->name }}</p>
                        ...
                        <td class="px-8 py-6 text-slate-600">{{ $organizer->email }}</td>
                        <td class="px-8 py-6">
                        ...
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.organizers.edit', $organizer) }}" class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-bold text-sm hover:bg-indigo-100 transition">
                                    Edit
                                </a>
                                @if(!$organizer->isSuperAdmin())
                                    <form action="{{ route('admin.organizers.destroy', $organizer) }}" method="POST" onsubmit="return confirm('Hapus akun ini?')">
                                        @csrf @method('DELETE')
