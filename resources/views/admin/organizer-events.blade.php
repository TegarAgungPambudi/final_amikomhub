...
@section('page_subtitle', 'Daftar event milik organisasi Anda')
            ...
            <tbody class="divide-y border-t">
                @forelse($events as $event)
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-8 py-6 font-bold">{{ $event->title }}</td>
                        <td class="px-8 py-6">
                            <span class="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-xs font-bold">
                                {{ $event->category->name ?? '-' }}
                            </span>
                        </td>
                        <td class="px-8 py-6 text-slate-600">{{ $event->date ? $event->date->format('d M Y') : '-' }}</td>
                        <td class="px-8 py-6 font-black text-indigo-600">Rp {{ number_format($event->price, 0, ',', '.') }}</td>
                        <td class="px-8 py-6">{{ $event->stock }}</td>
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-1">
                                @php $avg = $event->averageRating(); @endphp
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <= $avg)
                                    ...
                                    @endif
                                @endfor
                                <span class="text-xs text-slate-400 ml-1">({{ $event->reviews()->count() }} ulasan)</span>
                            </div>
                        </td>
                    </tr>
                @empty
