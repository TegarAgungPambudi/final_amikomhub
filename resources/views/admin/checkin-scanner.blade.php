...
@section('page_subtitle', 'Scan QR Code tiket peserta untuk verifikasi check-in.')
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="grid grid-cols-3 gap-4">
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <p class="text-2xl font-black text-indigo-600">{{ $totalTickets }}</p>
                <p class="text-xs text-slate-500 font-bold uppercase">Total Tiket Valid</p>
            </div>
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <p class="text-2xl font-black text-green-600">{{ $validTickets }}</p>
                <p class="text-xs text-slate-500 font-bold uppercase">Belum Check-in</p>
            </div>
            <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <p class="text-2xl font-black text-amber-600">{{ $usedTickets }}</p>
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
            ...
            <form action="{{ route('admin.checkin.manual') }}" method="POST" class="flex gap-3">
                @csrf
            ...
            <div class="space-y-3 max-h-[500px] overflow-y-auto">
                @forelse($recentCheckins as $checkin)
                    <div class="p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <p class="font-bold text-sm text-slate-800 truncate">{{ $checkin->customer_name }}</p>
                        <p class="text-xs text-slate-400 truncate">{{ $checkin->event->title ?? '-' }}</p>
                        <p class="text-[10px] text-green-600 font-bold mt-1">
                            {{ $checkin->used_at ? $checkin->used_at->format('H:i:s') : '-' }}
                        </p>
                    </div>
                @empty
