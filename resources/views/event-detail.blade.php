    ...
    <main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold">
                            {{ $event->organization ? strtoupper(substr($event->organization->name, 0, 2)) : ($event->category ? strtoupper(substr($event->category->name, 0, 2)) : '??') }}
                        </div>
                        <div>
                            @if($event->organization)
                                <a href="{{ route('organizer.profile', $event->organization->slug) }}" class="font-bold text-slate-800 hover:text-orange-600 transition">
                                    {{ $event->organization->name }}
                                </a>
                                <p class="text-xs text-slate-500">
                                    Verified Event ·
                                    <a href="{{ route('organizer.profile', $event->organization->slug) }}" class="text-orange-500 hover:underline">Lihat Profil</a>
                                </p>
                            @else
                                <p class="font-bold text-slate-800">{{ $event->category->name ?? 'Kategori tidak ditemukan' }}</p>
        ...
        <div class="lg:col-span-2 space-y-12">
            <div class="space-y-4">
                <span class="inline-flex px-4 py-1.5 bg-orange-100 text-orange-700 rounded-full text-sm font-bold uppercase tracking-wider">
                    {{ $event->category->name ?? 'Kategori tidak ditemukan' }}
                </span>
                <h1 class="text-4xl md:text-5xl font-black leading-tight text-slate-900">{{ $event->title }}</h1>
                <div class="flex flex-wrap gap-6 text-slate-500 font-medium">
                    <div class="flex items-center gap-2">
                        ...
                        <span>{{ $event->date ? $event->date->format('l, d M Y') : 'Tanggal belum diatur' }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @php $isPast = $event->date && $event->date->isPast(); @endphp
        <div class="bg-orange-600 rounded-[2.5rem] p-8 md:p-12 text-white shadow-2xl shadow-orange-200 relative overflow-hidden">
            ...
            <div class="space-y-8" id="reviews">
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-8">
                        <div class="text-center">
                            <div class="w-32 h-32 rounded-full bg-orange-50 flex items-center justify-center flex-col">
                                <span class="text-4xl font-black text-orange-600">{{ number_format($avgRating, 1) }}</span>
                                ...
                                <p class="text-xs text-slate-500 mt-1 font-medium">{{ $totalReviews }} ulasan</p>
                            </div>
                        </div>
                ...
                @auth
                            ...
                            <form action="{{ route('reviews.store', $event->id) }}" method="POST" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-1" id="star-rating">
                                        @for($i = 1; $i <= 5; $i++)
                                            <span class="star-btn cursor-pointer transition-all duration-150 {{ $userReview && $userReview->rating >= $i ? 'text-yellow-400' : 'text-slate-300' }}"
                                                  data-value="{{ $i }}"
                                                  onclick="setRating({{ $i }})"
                                ...
                                <div>
                                    ...
                                    <textarea name="comment" rows="4"
                                        ...
                                        placeholder="Bagikan pengalaman Anda menghadiri event ini...">{{ $userReview->comment ?? '' }}</textarea>
                                    @error('comment')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                    ...
                    @else
                        <div class="bg-slate-50 rounded-3xl border border-slate-200 p-8 text-center">
                            ...
                            <p class="text-sm text-slate-400 mt-1">Event ini akan berlangsung pada {{ $event->date->format('d M Y') }}</p>
                        </div>
                    @endif
                @else
                    <div class="bg-slate-50 rounded-3xl border border-slate-200 p-8 text-center">
                ...
                <div class="space-y-4">
                    <h4 class="font-bold text-lg text-slate-900">Ulasan Pengguna ({{ $totalReviews }})</h4>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modalLanjutBtn.addEventListener('click', function() {
        ...
        const checkoutUrl = '{{ url("checkout/" . $event->id) }}?email=' + encodeURIComponent(email);
