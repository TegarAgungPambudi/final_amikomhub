...
<div class="qr-section">
                    ...
                    <div class="qr-container">
                        @php
                            $qrPayloadEmail = $transaction->order_id ?? 'INVALID';
                            try {
                                $qrSvgEmail = \SimpleSoftwareIO\QrCode\Facades\QrCode::format('svg')
                                    ->size(150)
                                    ->margin(2)
                                    ->errorCorrection('M')
                                    ->generate($qrPayloadEmail);
                                $base64Qr = base64_encode($qrSvgEmail);
                                echo '<img src="data:image/svg+xml;base64,' . $base64Qr . '" alt="QR Code" width="150" height="150" style="display: block;">';
                            } catch (\Exception $e) {
                                echo '<img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=' . urlencode($qrPayloadEmail) . '" alt="QR Code" width="150" height="150" style="display: block;">';
                            }
                        @endphp
                    </div>
