...
<?php $__env->startSection('page_title', 'Dashboard Organizer'); ?>
<?php $__env->startSection('page_subtitle', $organization ? 'Organisasi: ' . $organization->name : 'Dashboard Organizer'); ?>

<?php $__env->startSection('content'); ?>
<!-- Header -->
<header class="flex justify-between items-center mb-10">
    <div>
        <h1 class="text-3xl font-black">Dashboard <?php echo e($organization->name ?? 'Organizer'); ?></h1>
    ...
    <div class="flex items-center gap-4">
        <div class="text-right hidden md:block">
            ...
            <p class="text-xs text-slate-400"><?php echo e($organization->name ?? 'Organizer'); ?></p>
        </div>
        <div class="w-12 h-12 bg-white rounded-2xl shadow-sm border flex items-center justify-center p-1">
            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(Auth::user()->name)); ?>&background=6366f1&color=fff" class="rounded-xl">
        </div>
    </div>
</header>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        ...
        <canvas id="revenueChart" height="120"
            data-monthly-labels='<?php echo e(json_encode($monthlyLabels ?? [])); ?>'
            data-monthly-revenue='<?php echo e(json_encode($monthlyRevenue ?? [])); ?>'></canvas>
    </div>
    <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        <h3 class="font-bold text-sm mb-3">🏆 Event Terpopuler</h3>
        <canvas id="eventsChart" height="120"
            data-event-labels='<?php echo e(json_encode($eventLabels ?? [])); ?>'
            data-event-data='<?php echo e(json_encode($eventData ?? [])); ?>'></canvas>
    </div>
</div>
...
<div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
    <div class="p-8 border-b flex justify-between items-center">
        ...
        <a href="<?php echo e(route('admin.organizer.transactions')); ?>" class="text-indigo-600 font-bold hover:underline">Lihat Semua</a>
    </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\laragon\www\finalproject_amikomhub\resources\views/admin/organizer-dashboard.blade.php ENDPATH**/ ?>